...
export default function DocumentUploadChatbot({ candidateName, recordId, error }: Props) {
  ...
  const MIN_SPEECH_DURATION_MS = 4000; // Only interrupt after 1.5 seconds of continuous speech
  ...
  if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
    wsRef.current.send(JSON.stringify({
      ...
      prompt: `You are an AI onboarding assistant helping ${candidateName} with their first week at the company. 
...
After delivering this message, wait for the user to respond or upload their document.`
    }));
  }
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px', background: '#f9fafb' }}>
          {messages.map((message) => (
            <div key={message.id} style={{ marginBottom: '16px', display: 'flex', justifyContent: message.type === 'agent' ? 'flex-start' : 'flex-end' }}>
              <div style={{
                background: message.type === 'agent' ? 'white' : '#005A9C',
          ...
          <div style={{ padding: '24px', background: 'white', borderTop: '1px solid #e5e7eb' }}>
            ...
            <button
              ...
              style={{
                width: '100%',
                padding: '16px',
                background: (isVerifying || !isConnected) ? '#e5e7eb' : '#005A9C',
                color: 'white',
                border: 'none',
                borderRadius: '12px',
                fontSize: '15px',
...
Thank you for your time and I wish you a great time with your wonderful team.
            ...
            <button
              ...
              style={{ padding: '14px 40px', background: '#005A9C', color: 'white', border: 'none', borderRadius: '10px', fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}
            >
              Done
            </button>
          </div>
