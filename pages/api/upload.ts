...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log(`Uploading ${documentType} to Blob`);

    // Upload to Vercel Blob
    const fileBuffer = fs.readFileSync(file.filepath);
    const filename = `${recordId}_${documentType}_${Date.now()}_${file.originalFilename}`;
    ...
    const response = await fetch(
      `https://api.airtable.com/v0/appMvECrw7CrJFCO0/tblqaH9RrTO6JuG5N/${recordId}`,
      {
        ...
        body: JSON.stringify({
          fields: {
            [fieldIds[documentType]]: [{ url: blob.url, filename: file.originalFilename }]
          }
        })
      }
    );
